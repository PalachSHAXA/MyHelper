...
async def admin_help(message: Message):
    ...
    # if user_id in boss:
    await message.reply(
        "/id - пришлёт вам ваш телеграм ID\n"
        "/register_admin - зарегистрирует администратора\n"
        "/delete_admin - удалит себя, если вы введёте команду, или удалит администратора по ID\n"
        "/delete - то же самое, только для пользователей\n"
        "/sendall - отправит сообщение всем пользователям, которые хотя бы раз нажимали /start\n"
        "/send_branch - отправит рассылку на указанный филиал\n"
        "/get_user - выведет всю информацию о пользователе из базы данных (нужно ввести его ID)\n"
        "/get_admin - работает так же, как и /get_user, только для администраторов\n"
        "/exel - пришлёт файл Excel со всеми зарегистрированными в базе данных пользователями"
    )
    # else:
    #     await message.answer('error 404')
